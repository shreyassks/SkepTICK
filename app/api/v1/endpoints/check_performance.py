...
class BollingerBandsStrategy(Strategy):
    def init(self):
        close = self.data.Close
        self.bollinger = self.I(
            ta.volatility.BollingerBands, pd.Series(close), window=20)
...
def save_graph(output, nifty_50_data, strategy_cagr, nifty_50_cagr, implement_strat):
    strategy_series = output['_equity_curve'][['Equity']].reset_index().rename(
        columns={'index': 'Date', 'Equity': 'Strategy'})
    nifty_50_series = nifty_50_data['Close'].reset_index().rename(
        columns={'Close': 'Nifty50_Index'})
    ...
    plot_df['Nifty50_return'] = (
        plot_df['Nifty50_Index']-plot_df['Nifty50_Index'].shift(1))/plot_df['Nifty50_Index'].shift(1)

    # Calculate Nifty 50 investment values
    initial_investment = 100000
    investment_values_nifty = [initial_investment]
    for i in range(1, len(plot_df)):
        investment_value = investment_values_nifty[i -
                                                   1] * (1 + plot_df['Nifty50_return'][i])
...
def breakdown(request: PerfRequest):
    ...
    try:
        ...
        bt = Backtest(df, selected_strategy, cash=100000,
                      commission=0.002, exclusive_orders=True)
        output = bt.run()
        nifty_50_data = yf.download('^NSEI', period=f'{duration}y')
        nifty_50_cagr = ((nifty_50_data.tail(1)[
                         'Close'].values[0] / nifty_50_data.head(1)['Close'].values[0]) ** (1 / 3) - 1)*100
        strategy_cagr = output['Return (Ann.) [%]']

        image_name = save_graph(output, nifty_50_data,
                                strategy_cagr, nifty_50_cagr, implement_strat)
